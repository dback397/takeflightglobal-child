...
final class VerificationToken
{
    ...
    public static function tfgCreateVerificationToken(WP_REST_Request $request)
    {
        ...
        $email   = Utils::normalizeEmail($raw_em);
        ...
        $result = self::createVerificationToken($email, $code, $name, $source, $consent, $seq, $seq_code);
...
/**
    ...
    public static function createVerificationToken(
        ...
        if ($sequence_id === null) {
            $sequence_id   = Sequence::next('newsletter_seq', 1);
            $sequence_code = Sequence::format($sequence_id, 'N', 6);
        } elseif ($sequence_code === null) {
            $sequence_code = Sequence::format((int) $sequence_id, 'N', 6);
        }
...
\class_alias(\TFG\Features\MagicLogin\VerificationToken::class, 'TFG_Verification_Token');
