...
final class NewsletterSubscription
{
    ...
    public static function render_newsletter_signup_form($atts = [], $content = null, $tag = ''): string
    {
        ...
        $prefill_email = Utils::normalizeEmail($atts['prefill_email']);
        if (!$prefill_email && !empty($_COOKIE['tfg_prefill_email'])) {
            $prefill_email = Utils::normalizeEmail(\wp_unslash($_COOKIE['tfg_prefill_email']));
            // Clear the cookie so it doesn't linger
            if (\class_exists(Cookies::class) && \method_exists(Cookies::class, 'delete_ui_cookie')) {
                Cookies::deleteUiCookie('tfg_prefill_email');
            }
        }
    ...
    public static function handle_newsletter_signup(): void
    {
        ...
        if (\class_exists(FormRouter::class) && \method_exists(FormRouter::class, 'matches')) {
            // if your router is present, let it decide
            $matches = FormRouter::matches('newsletter_signup');
        }
        ...
        // ── INTAKE / NORMALIZE ───────────────────────────────────────────────
        $vcode   = Utils::normalizeToken($_POST['verification_code'] ?? '');
        $email   = Utils::normalizeEmail(\wp_unslash($_POST['subscriber_email'] ?? ''));
...
// Legacy alias (optional) if you still reference the old class name elsewhere
\class_alias(\TFG\Features\Newsletter\NewsletterSubscription::class, 'TFG_Newsletter_Subscription');
