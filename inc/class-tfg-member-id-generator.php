...
class TFG_Member_ID_Generator {
     ...
     */
    public static function get_next_id(string $member_type): string|false {
        $cfg = self::config($member_type);
        if (!$cfg) return false;

        $n = self::atomic_next($cfg['option'], 0);
     ...
     */
    public static function preview_next_id(string $member_type): string|false {
        $cfg = self::config($member_type);
        if (!$cfg) return false;
    ...
    /**
     * (Optional) Set the counter explicitly (e.g., from an admin screen).
     * Pass the raw integer "last used" value (the next call will return +1).
     */
    public static function set_counter(string $member_type, int $value): bool {
        $cfg = self::config($member_type);
        if (!$cfg) return false;
     ...
     */
    public static function get_counter(string $member_type): int|false {
        $cfg = self::config($member_type);
        if (!$cfg) return false;
     ...
     */
    private static function atomic_next(string $option_name, int $start = 0): int|false {
        global $wpdb;
