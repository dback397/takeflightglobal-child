...
class TFG_Error_Modal {

    /**
     * Usage: echo do_shortcode('[tfg_error_modal code="104"]');
     */
    public static function init(): void {
        add_shortcode('tfg_error_modal', [__CLASS__, 'render_shortcode']);
        self::register_code_shortcode(); // <-- make sure this is registered
        add_action('wp_footer', [__CLASS__, 'inject_modal_to_footer']);
    }
    ...
    public static function inject_modal_to_footer(): void {
        ...
        if ($code) {
            ...
            echo self::render_shortcode(['code' => $code]); // safe: our HTML is inert; JS fills content
        } else {
            // Still inject the container so JS can use window.tfgErrorMessages later if needed
    ...
    public static function show_r($code, int $seconds = 30, ?string $redirect_url = null): void {
        ...
        // Scope the transient to the current visitor
        set_transient(self::key_for_user(), $code, max(5, $seconds));

        // Safe redirect with no-cache headers; guard headers already sent
        if (!headers_sent()) {
            nocache_headers();
        ...
        // Fallback: inject immediately if we can’t redirect
        echo self::show($code);
    }
}
