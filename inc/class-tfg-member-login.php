...
class TFG_Member_Login {

    public static function init(): void {
        // Shortcodes
        add_shortcode('tfg_member_login_form',        [self::class, 'render_login_form']);
        add_shortcode('tfg_member_reset_request',     [self::class, 'render_reset_request_form']); // identity check
        add_shortcode('tfg_member_reset_form',        [self::class, 'render_password_reset_form']); // requires cookie
        add_shortcode('tfg_forgot_member_id_form',    [self::class, 'render_forgot_member_id_form']);

        // Handlers
        add_action('init', [self::class, 'handle_login']);
        add_action('init', [self::class, 'handle_reset_request']);   // handles identity check
        add_action('init', [self::class, 'handle_password_reset']);  // actual password write
        add_action('init', [self::class, 'handle_forgot_member_id']);
    ...
    public static function render_login_form(): string {
        // Bounce if already “logged in” via cookie
        if (TFG_Cookies::get_member_id() && !headers_sent()) {
            nocache_headers();
            wp_safe_redirect(site_url('/member-dashboard/'));
            exit;
        }

        $is_member_ui = (bool) TFG_Cookies::get_member_id();
    ...
    public static function handle_login(): void {
        if (!TFG_Form_Router::matches('member_login')) return;
        if (empty($_POST['tfg_member_login_submit'])) return;
        if (!isset($_POST['_tfg_nonce']) || !wp_verify_nonce($_POST['_tfg_nonce'], 'tfg_member_login')) {
            echo '<p class="tfg-error">Security check failed. Please refresh and try again.</p>';
            return;
        }

        $member_id = TFG_Utils::normalize_member_id(wp_unslash($_POST['member_id'] ?? ''));
        ...
        // Optional email (for stronger HMAC binding in TFG_Cookies)
        $email = function_exists('get_field')
            ? TFG_Utils::normalize_email(get_field('contact_email', $profile_id) ?: '')
            : TFG_Utils::normalize_email(get_post_meta($profile_id, 'contact_email', true));

        // Set trusted session cookie
    ...
    public static function render_reset_request_form(): string {
        ob_start(); ?>
    ...
    public static function handle_reset_request(): void {
        if (!TFG_Form_Router::matches('password_reset_request')) return;
        if (empty($_POST['tfg_member_reset_request_submit'])) return;
        if (!isset($_POST['_tfg_nonce']) || !wp_verify_nonce($_POST['_tfg_nonce'], 'tfg_member_reset_request')) {
            echo '<p class="tfg-error">Security check failed. Please refresh and try again.</p>';
            return;
        }

        $member_id = TFG_Utils::normalize_member_id(wp_unslash($_POST['reset_member_id'] ?? ''));
        $email     = TFG_Utils::normalize_email(wp_unslash($_POST['reset_email'] ?? ''));
        ...
        // Check against profile_stub (your original logic)
        $stub = get_posts([
        ...
        // If you wanted pre-auth reset via token, you’d generate+email a reset token here.
        // For now, we simply render the reset form (but it still requires a member cookie).
        echo self::render_password_reset_form();
    }
    ...
    public static function render_password_reset_form(): string {
        // Require that the user is “logged in” as member (cookie present)
        $member_id = TFG_Cookies::get_member_id();
        if (!$member_id) {
    ...
    public static function handle_password_reset(): void {
        if (!TFG_Form_Router::matches('password_reset')) return;
        if (empty($_POST['tfg_submit_password'])) return;
        if (!isset($_POST['_tfg_nonce']) || !wp_verify_nonce($_POST['_tfg_nonce'], 'tfg_member_password_reset')) {
            echo '<p class="tfg-error">Security check failed. Please refresh and try again.</p>';
            return;
        }

        // Must have a member cookie to reset
        $member_id = TFG_Cookies::get_member_id();
        if (!$member_id) {
        ...
        // Update on both stub and profile (if they exist)
        $updated_any = false;
        ...
        if ($updated_any) {
            // Clear trusted cookie to force re-login
    ...
    public static function render_forgot_member_id_form(): string {
        ob_start(); ?>
    ...
    public static function handle_forgot_member_id(): void {
        if (!TFG_Form_Router::matches('forgot_member_id')) return;
        if (empty($_POST['tfg_member_id_lookup_submit'])) return;
        if (!isset($_POST['_tfg_nonce']) || !wp_verify_nonce($_POST['_tfg_nonce'], 'tfg_member_forgot_id')) {
            echo "<p class='tfg-error'>Security check failed. Please refresh and try again.</p>";
            return;
        }
        ...
        // Prefer emailing the member ID instead of printing it:
        // TFG_Mailer::send($email, 'Your Member ID', 'member_id_reminder', ['member_id' => $member_id]);
