...
class TFG_Prefill
{
    ...
    public static function init(): void
    {
        ...
        add_filter('wpforms_field_value_tfg_acf_programs', [__CLASS__, 'prefill_programs'], 10, 3);
        add_filter('wpforms_field_value_tfg_acf_gdpr',     [__CLASS__, 'prefill_gdpr'],     10, 3);

        // Checkbox default ticking for one form
        add_filter('wpforms_frontend_form_data', [__CLASS__, 'prefill_checkboxes'], 10, 3);
    }

    /* ---------- Core helpers ---------- */

    private static function acf_available(): bool
    {
    ...
    private static function acf_get(string $key, int $post_id)
    {
        if (isset(self::$cache[$post_id][$key])) {
            return self::$cache[$post_id][$key];
        }

        $val = self::acf_available() ? get_field($key, $post_id) : get_post_meta($post_id, $key, true);
    ...
    public static function prefill_programs($value, $field_id = null, $form_data = null)
    {
        ...
        if (!$post_id || !self::can_read_post($post_id)) {
            return $value;
        }
        $acf = self::acf_get(self::ACF_FIELD_PROGRAMS, $post_id);
        return self::normalize_scalar($acf) ?: $value;
    }

    // For WPForms field with Dynamic Population: Filter = tfg_acf_gdpr
    public static function prefill_gdpr($value, $field_id = null, $form_data = null)
    {
        $post_id = self::current_post_id();
        if (!$post_id || !self::can_read_post($post_id)) {
            return $value;
        }
        $acf = self::acf_get(self::ACF_FIELD_GDPR, $post_id);
     ...
     */
    public static function prefill_checkboxes($form_data, $fields, $form_id)
    {
        // Structure guards
